...
export default class Recovery {
     ...
     * @returns 
     */
    public async getTotalVotes(vaultAddress: string, recoverProcessId: string): Promise<number> {
        ...
        for (const guardian of guardians) {
            let owner = await this.getGuardianVote(vaultAddress, recoverProcessId, guardian);
            if (this.web3.utils.isAddress(owner)) totalVotes++;
        }
        return totalVotes;
    }
     ...
     * @returns 
     */
    public async canRecover(vaultAddress: string, recoverProcessId: string, newOwner: string): Promise<boolean> {
        // get all processIds for a Recovery Vault
        let processIds = await this.getRecoverProcessesIds(vaultAddress);
        // search for our processId
        let index = processIds.findIndex(id => id == recoverProcessId);
